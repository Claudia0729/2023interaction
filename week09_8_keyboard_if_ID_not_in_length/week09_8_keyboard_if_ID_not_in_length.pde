...
void draw()
{
  ...
  for(int i=0;i<26;i++)
  {
    if(ID<typing.length() && typing.charAt(ID)-'a'==i) rect(pos[i][0],pos[i][1],28,30,5);//ID沒有超過長度 
  ...
  if( ID<typing.length() ) text(typed+typing.charAt(ID), 50, 100);//現在要打的字 是typing.charAt(0) 且ID沒有超過長度
...
void keyPressed()
{
  if(key>='a' && key<='z')
  {
    if( ID<typing.length() && key == typing.charAt(ID) )//字母正確  就往下 且 ID沒有超過長度
    {
      pressed[key-'a']=true;
      typed += key;
      ID ++;
    ...
    {
      background(255,0,0);//字母錯誤就畫面 閃一下(紅色)
    }
