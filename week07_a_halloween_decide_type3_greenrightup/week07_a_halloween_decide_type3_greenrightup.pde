...
void draw(){
  ...
  strokeWeight(4);//線的粗度
  float dpx = 0, dpy = 0;//要看 p.x p2.x的差距 p.y p2.y的差距
  ...
  else if (rightdown>150 ) stroke(0,255,0);//type3 前半段 往右下
  else if ( green_ok ) stroke(0,255,0);//type3 的前半段 往右下
  else if ( green_part1 ) stroke(0,128,0);//暗暗的綠色 typee的前半段 往右下
